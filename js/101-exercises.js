...
function addToDone(message) {
    ...
    var domAnswersList = document.querySelector(".correct ul");
    var domCount = document.querySelector("#count");
    var domFinished = document.querySelectorAll('.finished');
...
const isPositiveEven = (n) => n % 2 == 0 && n > 0;
...
const isNegativeEven = (n) => -n % 2 == 0 && n < 0;
...
const isMultipleOfBothThreeAndFive = (n) => isMultipleOfThree(n) && isMultipleOfFive(n);
...
const quotient = (n1, n2) => Math.floor(n1 / n2);
...
const remainder = (n1, n2) => Math.floor(n1 % n2);
...
const hasVowels = (word) => /[aeiou]/.test(word);
...
const startsWithVowel = (word) => isVowel(word.slice(0, 1));
...
const endsWithVowel = (word) => isVowel(word.slice(-1));
...
const startsAndEndsWithVowel = (word) => startsWithVowel(word) && endsWithVowel(word);
...
const first = (values) => Array.from(values)[0];
...
const second = (values) => Array.from(values).slice(1, 2)[0];
...
const third = (values) => Array.from(values).slice(2, 3)[0];
...
const forth = (values) => Array.from(values).slice(3, 4)[0];
...
const last = (values) => Array.from(values).slice(-1)[0];
...
const secondToLast = (values) => Array.from(values).slice(-2)[0];
...
const thirdToLast = (values) => Array.from(values).slice(-3)[0];
...
const firstToLast = (values) => [...values.slice(1), first(values)];
...
const sumAll = (args) => args.reduce((s, v) => s + v);
...
const mean = (args) => sumAll(args) / args.length;
...
const maxMinusMin = (args) => Math.max(...args) - Math.min(...args);
...
const productOfAll = (args) => args.reduce((a, b) => a * b);
...
const getHighestNumber = (args) => Math.max(...args);
...
const getSmallestNumber = (args) => Math.min(...args);
...
const onlyOddNumbers = (args) => args.filter(isOdd);
...
const onlyEvenNumbers = (args) => args.filter(isEven);
...
const onlyPositiveNumbers = (args) => args.filter(isPositive);
...
const onlyNegativeNumbers = (args) => args.filter(isNegative);
...
const hasEvens = (args) => args.some(isEven);
...
const countEvens = (args) => args.filter(isEven).length;
...
const hasOdds = (args) => args.some(isOdd);
...
const countOdds = (args) => args.filter(isOdd).length;
...
const countNegatives = (args) => args.filter(isNegative).length;
...
const countPositives = (args) => args.filter(isPositive).length;
...
const onlyPositiveEvens = (args) => args.filter(isPositive).filter(isEven);
...
const onlyPositiveOdds = (args) => args.filter(isPositive).filter(isOdd);
...
const onlyNegativeEvens = (args) => args.filter(isNegative).filter(isEven);
...
const onlyNegativeOdds = (args) => args.filter(isNegative).filter(isOdd);
...
const shortestString = (args) => args.reduce((f, l) => (f.length > l.length) ? l : f);
...
const longestString = (args) => args.reduce((f, l) => (f.length < l.length) ? l : f);
...
const getUniqueValues = (args) => [...new Set(args)];
...
const getNumberOfBooks = (args) => args.length;
...
const totalOfBookPrices = (args) => sumAll(args.map(({ price }) => price));
...
const getAverageBookPrice = (args) => mean(args.map(({ price }) => price));
...
const highestPriceBook = (args) => args.reduce((prev, curr) => (prev.price < curr.price) ? curr : prev);
...
const lowestPriceBook = (args) => args.reduce((prev, curr) => (prev.price > curr.price) ? curr : prev);
...
const numberOfItemTypes = (args) => [...new Set(args.items)].length;
...
const totalNumberOfItems = (args) => sumAll(args.items.map((item) => item.quantity));
...
const getAverageItemPrice = (args) => mean(args.items.map((item) => item.price));
...
const getAverageSpentPerItem = (args) => {
    const avgItemPrices = sumAll([...args.items.map((item) => (item.price * item.quantity))]);
...
const mostSpentOnItem = (args) => args.items.reduce((prev, curr) => (Math.max(prev.price * prev.quantity) > Math.max(curr.price * curr.quantity)) ? prev : curr);
