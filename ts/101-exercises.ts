...
const assert = function(actual: any, expected: any, message = "") {
...
function addToDone(message: string) {
    var node: HTMLElement = document.createElement("li");                 // Create a <li> node
    var textnode: Text = document.createTextNode(message);         // Create a text node
    var domAnswersList = document.querySelector(".correct ul") as HTMLElement;
    var domCount = document.querySelector("#count") as HTMLElement;
    var domFinished: NodeListOf<Element> = document.querySelectorAll('.finished');
    
    node.classList.add("finished");
    node.appendChild(textnode)
    domAnswersList.appendChild(node); // Append the text to <li>
    domCount.innerHTML = domFinished.length + " of 101";
}
...
const doingJSRightNow: boolean = true;
...
const onMarsRightNow: boolean = false;
...
const fruits: Array<string> = ["mango","banana","guava","kiwi","strawberry"];
...
const vegetables: Array<string> = ["eggplant", "broccoli", "carrot", "cauliflower", "zucchini"];
...
const reverseSort = [...fruits].sort().reverse();
assert(reverseSort, ['tomato', 'strawberry', 'mango', 'kiwi', 'guava', 'banana'], "Exercise 9")
...
const fruitsAndVeggies = [...reverseSort, ...vegetables]
...
function randomPositiveEvenNumber(): number {
...
function randomPositiveOddNumber(): number {
    var randomNumber = Math.ceil(Math.random() * 100) + 10;
    if(randomNumber % 2 === 0) {
...
function randomNegativeEvenNumber(): number {
    var randomNumber = Math.ceil(Math.random() * -100) - 10;
    if(randomNumber % 2 === 0) {
        return randomNumber;
    }
...
function randomNegativeOddNumber(): number {
    var randomNumber = Math.ceil(Math.random() * -100) - 10;
    if(randomNumber % 2 === 0) {
...
const plusTwo = (x: number): number => x + 2;
...
const isPositiveOdd = (n:number): boolean => n % 2 >= 1;
...
const isPositiveEven = (n:number): boolean => n % 2 == 0 &&  n > 0;
...
const isNegativeOdd = (n:number): boolean => n % 2 <= -1;
...
const isNegativeEven = (n: number): boolean => -n % 2 == 0 && n < 0
...
const reverseSign = (n: number): number => n * -1;
...
const absoluteValue = (n: number): number => Math.abs(n);
...
const isMultipleOfBothThreeAndFive = (n: number): boolean => isMultipleOfThree(n) && isMultipleOfFive(n);
...
const quotient = (n1: number, n2: number): number => Math.floor(n1 / n2);
...
const remainder = (n1: number, n2: number): number => Math.floor(n1 % n2);
...
const sumOfSquares = (n1: number, n2: number): number => square(n1) + square(n2);
...
const timesTwoPlusThree = (n: number): number => double(n) + 3;
...
const areaOfRectangle = (w: number, l: number): number => w * l;
...
const areaOfCircle = (r: number): number => square(r) * Math.PI;
...
const circumference = (n: number): number => double(n) * Math.PI;
...
const isVowel = (letter: string): boolean => ['a', 'e', 'i', 'o', 'u'].includes(letter.toLowerCase())
...
const hasVowels = (word: string): boolean => /[aeiou]/.test(word);
...
const removeVowels = (word: string): string => word.replace(/[aeiou]/gi, '');
...
const startsWithVowel = (word: string): boolean => isVowel(word.slice(0,1));
...
const endsWithVowel = (word: string): boolean => isVowel(word.slice(-1))
...
const startsAndEndsWithVowel = (word: string): boolean => startsWithVowel(word) && endsWithVowel(word);
...
const first = (values: any): any => Array.from(values)[0];
...
const second = (values: any): any => Array.from(values).slice(1,2)[0]
...
const third = (values: any): any => Array.from(values).slice(2,3)[0];
...
const forth = (values: any): any => Array.from(values).slice(3,4)[0];
...
const last = (values: any): any => Array.from(values).slice(-1)[0]
...
const secondToLast = (values: any): any => Array.from(values).slice(-2)[0]
...
const thirdToLast = (values: any): any => Array.from(values).slice(-3)[0]
...
const firstAndSecond = (values: any): any[] => [first(values), second(values)];
...
const firstAndLast = (values: any): any[] => [first(values), last(values)];
...
const firstToLast = (values: any): any[] => [...values.slice(1),first(values)];
...
const sumAll = (args: number[]): number => args.reduce( (s,v) => s + v);
...
const mean = (args: number[]): number => sumAll(args) / args.length;
...
const median = (args: number[]): number => {
    const mid = Math.floor(args.length / 2)
    const values = [...args].sort( (a,b) => a - b)
    return args.length % 2 !== 0 ? values[mid] : (values[mid - 1] + values[mid]) /2;
}
...
const maxMinusMin = (args: number[]): number => Math.max(...args) - Math.min(...args);
...
const productOfAll = (args: number[]): number => args.reduce( (a,b) => a * b);
...
const getHighestNumber = (args: number[]): number => Math.max(...args);
...
const getSmallestNumber = (args: number[]): number => Math.min(...args);
...
const onlyOddNumbers = (args: number[]): number[] => args.filter(isOdd);
...
const onlyEvenNumbers = (args: number[]): number[] => args.filter(isEven);
...
const onlyPositiveNumbers = (args: number[]): number[] => args.filter(isPositive);
...
const onlyNegativeNumbers = (args: number[]): number[] => args.filter(isNegative);
...
const hasEvens = (args: number[]): boolean => args.some(isEven);
...
const countEvens = (args: number[]): number => args.filter(isEven).length;
...
const hasOdds = (args: number[]): boolean => args.some(isOdd);
...
const countOdds = (args: number[]): number => args.filter(isOdd).length;
...
const countNegatives = (args: number[]): number => args.filter(isNegative).length;
...
const countPositives = (args: number[]): number => args.filter(isPositive).length;
...
const onlyPositiveEvens = (args: number[]): number[] => args.filter(isPositive).filter(isEven);
...
const onlyPositiveOdds = (args: number[]): number[] => args.filter(isPositive).filter(isOdd);
...
const onlyNegativeEvens = (args: number[]): number[] => args.filter(isNegative).filter(isEven);
...
const onlyNegativeOdds = (args: number[]): number[] => args.filter(isNegative).filter(isOdd);
...
const shortestString = (args: string[]): string => args.reduce( (f, l) => (f.length > l.length) ? l : f);
...
const longestString = (args: string[]): string => args.reduce( (f, l) => (f.length < l.length) ? l : f);
...
const getUniqueValues = (args: string[]): string[] => [...new Set(args)];
...
const elementsTimesTwo = (args: number[]): number[] => args.map(double);
...
const flatten = (args: any[]): any[] => args.flat(Infinity);
...
const addOneToArray = (args: number[]): number[] => args.map(addOne);
...
const getPaperTitle = (obj: Paper): string | undefined => obj.title;
...
const getYearPublished = (obj: Paper): number | undefined => obj.year_published;
...
const getPrice = (obj: Book): number | undefined => obj.price;
...
const getBookAuthor = (obj: Book): string | undefined => obj.author;
...
const getNumberOfBooks = (args: Array<Book>): number => args.length;
...
const totalOfBookPrices = (args: Array<Book>): number => sumAll(args.map( ({price}): number => price as number));
...
const getAverageBookPrice = (args: Array<Book>): number => mean(args.map( ({price}): number => price as number));
...
const highestPriceBook = (args: Array<Book>): any => args.reduce( (prev: any, curr: any): any => (prev.price < curr.price) ? curr : prev)
...
const lowestPriceBook = (args: Array<Book>): any => args.reduce( (prev: any, curr: any): any => (prev.price > curr.price) ? curr : prev)
...
const getTaxRate = (obj: any): number => obj.tax;
...
const numberOfItemTypes = (args: any): number => [...new Set(args.items)].length;
...
const totalNumberOfItems = (args: any): number => sumAll(args.items.map( ( item: any) => item.quantity));
...
const getAverageItemPrice = (args: any): number => mean(args.items.map( (item:any) => item.price));
...
const getAverageSpentPerItem = (args: any): number => {
    const avgItemPrices = sumAll([...args.items.map( (item: any) => (item.price * item.quantity))])
    return avgItemPrices / totalNumberOfItems(args)
}
...
const mostSpentOnItem = (args: any): Item => 
    args.items.reduce( (prev: any, curr: any): any =>
         (Math.max(prev.price * prev.quantity) > Math.max(curr.price * curr.quantity)) ? prev: curr)
